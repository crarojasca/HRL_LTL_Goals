...
def loadGameModule(args, trainname):
    ...
    try:
        ...
        if True:
            ...
            game = mod.CocktailParty(trainsessionname=trainname,rows=args.rows,cols=args.cols)
            game.differential = True
        elif (args.game=='CPD'):
            mod = importlib.import_module('CocktailPartyDynamic')
            game = mod.CocktailParty(trainsessionname=trainname)
            game.differential = False
        elif (args.game=='CPR'):
            ...
            game = mod.PickAndPlace(trainsessionname=trainname)
        elif (args.game=='PPx'):
            mod = importlib.import_module('PickAndPlace')
            game = mod.PickAndPlace(trainsessionname=trainname)
            game.RA_exploration_enabled = True
...
def save():
    if game is not None and agent is not None and (not args.eval):
        # filename = trainname +"_%05d" % (self.iteration)
        filename = f'data/{trainname}/model'
...
def writeinfo(args, trainname, game, agent, init=True, optimalPolicyFound=False):
    infofile = open(f"data/{trainname}/"+ "info","a+")
    allinfofile = open("data/all_info","a+")
    ...
    if (init):
        ...
        infofile.write("\n%s,%s,%s,%d,%d,%s,%.3f,%.3f,%.3f,%d,%.3f,%d,%d,%.2f,%d,%.2f,%d,%d\n\n" %(strtime,trainname,args.game,args.rows,args.cols,agent.name,agent.gamma,agent.epsilon, agent.alpha,agent.nstepsupdates,agent.lambdae,game.iteration,game.score,game.cumreward, game.numactions,game.hireward,game.hiscore,game.elapsedtime))
...
# learning process
def learn(args, game, agent, logger, maxtime=-1, stopongoal=False, fn_step=execution_step):
...
if __name__ == "__main__":
    ...
    if (game.iteration==0):
        writeinfo(trainname,game,agent,init=True)
    ...
    if (args.eval):
        ...
        writeinfo(trainname,game,agent,init=False)
