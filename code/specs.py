...
class Spec_Controller:
    ...
    def step(self, labels):
        ...
        for j in range(self.num_specs):
            if not labels[j]:
                self.states[j], _ = self.specs[j].ldba.step(labels[j])
                self.acceptances[j] = False
            else:
                self.states[j], self.acceptances[j] = self.specs[j].ldba.step(labels[j])
...
class LDBA:
    ...
    def __init__(self, ltl):
        ...
        rabinizer_dir='/home/crarojasca/Monash/HRL_LTL_Goals/almanac/src/rabinizer4/bin/ltl2ldba'
        out=check_output([rabinizer_dir, '-e', ltl])
