...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, args):
        ...
        if args.has_continuous_action_space:
            ...
            self.action_var = torch.full((action_dim,), args.action_std.init * args.action_std.init).to(device)
        # actor
        if args.has_continuous_action_space :
            self.actor = nn.Sequential(
                            nn.Linear(state_dim.shape[0], 64),
                            nn.Tanh(),
                            nn.Linear(64, 64),
                            nn.Tanh(),
...
class PPO:
    def __init__(self, observation_space, action_space, args=None):
        ...
        if args.has_continuous_action_space:
            self.action_std = args.action_std.init

        self.action_std_cfg = args.action_std
        ...
        self.policy = ActorCritic(observation_space, action_space, args).to(device)
        self.number_parameters = sum(p.numel() for p in self.policy.parameters())
        self.optimizer = torch.optim.Adam([
                        {'params': self.policy.actor.parameters(), 'lr': args.lr_actor},
                        {'params': self.policy.critic.parameters(), 'lr': args.lr_critic}
                    ])

        self.policy_old = ActorCritic(observation_space, action_space, args).to(device)
        ...
        if args.has_continuous_action_space:
            ...
            print("--------------------------------------------------------------------------------------------")
            print("starting std of action distribution : ", self.action_std.init)
            print("decay rate of std of action distribution : ", self.action_std.decay_rate)
            print("minimum std of action distribution : ", self.action_std.min)
            print("decay frequency of std of action distribution : " + str(self.action_std.decay_freq) + " timesteps")
        else:
    ...
    def run(self, env, logger=None):
        ...
        while self.episodes < self.max_episodes:
            ...
            while not done and ep_steps < self.max_steps_ep:
                ...
                # update PPO agent
                if steps % self.update_timestep == 0:
                    loss = self.update()

                # if continuous action space; then decay action std of ouput action distribution
                if self.has_continuous_action_space and steps % self.action_std_cfg.decay_freq == 0:
                    self.decay_action_std(self.action_std_cfg.decay_rate, self.action_std_cfg.min)


                # Log step
                if logger:
                    logger.log_data(
                        steps, reward, loss, 0, 0, 0)
