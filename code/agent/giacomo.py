...
class RLAgent(object):
    ...
    def updateQ_n(self,kn,x2): # n-steps Q update
        ...
        if (self.debug):
            ...
            if not x2 is None: print("Q[%d] = %.3f" %(x2,Qx2))
    ...
    # learning process
    def learn(self, args, game, agent, logger, maxtime=-1, stopongoal=False):
        ...
        while (run and (args.niter<0 or game.iteration<=args.niter)):
            ...
            if (game.finished):
                ...
                rewards += [game.cumreward]
                mean_reward = np.mean(rewards[-100:])
                logger.log_episode(game.iteration, game.cumreward, mean_reward, {}, game.elapsedtime, 0)
                time.sleep(game.sleeptime)
